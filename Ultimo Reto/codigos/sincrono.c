...
int main(int argc, char** argv) {
    ...
    for (int iteracion = 1; iteracion <= MAX_ITERATIONS; iteracion++) {
        ...
        // Comunicar el número local de autos movidos a todos los procesos de manera sincrónica
        for (int i = 0; i < size; i++) {
            if (i != rank) {
                MPI_Send(&movimientosLocal, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
                MPI_Recv(&movimientosLocal, 1, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
